...
def build_b_vec_1(phi_lst, phi_prime_lst,
    ...
    for i in range(len(phi_lst)):
        ...
        b_vec_1[i] -= inner_product(first_term, phi_prima_i, patches)
...
def build_b_vec_2(phi_1st, phi_prime_1st, 
    ...
    b_vec = np.zeros(len(phi_1st), dtype=np.float64)
    patches = domain_partition[0]
    ...
    for i in range(len(phi_1st)):
        ...
        b_vec[i] = inner_product(f_func, phi_i, patches)-htc*(f_func(x_max)-u_b)*phi_i(x_max)-htc*(f_func(x_min)-u_a)*phi_i(x_min)
