...
def main():
    ...
    if make_plots and plant.use_multiprocessing or plant.rank == 0:
        ...
        (quant, time_unit) = decant_filt.filtration_filtrate_phase.get_quantity_history('solids-massfrac')

        quant.plot(x_scaling=1 / unit.minute, x_label='Time [m]',
                   y_label=quant.latex_name + ' [' + quant.unit + ']')
        plt.grid()
        plt.savefig('decant-filt-filtrate_mass-fraction.png', dpi=300)

        (quant, time_unit) = decant_filt.filtration_slurry_phase.get_quantity_history('solids-massfrac')

        quant.plot(x_scaling=1 / unit.minute, x_label='Time [m]',
                   y_label=quant.latex_name + ' [' + quant.unit + ']')
        plt.grid()
        plt.savefig('decant-filt-slurry_mass-fraction.png', dpi=300)
